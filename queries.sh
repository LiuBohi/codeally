...
WINNER_GOALS_TOTAL="$($PSQL "SELECT SUM(winner_goals) FROM games")"
echo $WINNER_GOALS_TOTAL
...
echo "$($PSQL "SELECT name FROM (SELECT winner_id FROM games WHERE year=2014 AND round='Eighth-Final' UNION SELECT opponent_id FROM games WHERE year=2014 AND round='Eighth-Final') AS t LEFT JOIN teams ON t.winner_id=teams.team_id ORDER BY name")"
